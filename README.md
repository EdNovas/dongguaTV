...
相当一部分源站 CDN 会**封 Cloudflare 机房出口 IP**或把播放地址签名绑定客户端 IP——这类线路**直连正常、经 worker 代理必 403/超时**（实测约 1/3 的直连站如此，worker 本体是好的）。播放失败时客户端自动分诊：

- **本站测速实测"直连可达"** → 判定为源站封代理：**原站直接回退直连**（不跳线路——下一条大概率同样封 CF，跳只会级联失败），弹提示"可能含广告"，并把该站记入 **3 天"屏蔽过滤代理"记账**（localStorage 持久化）：跨会话不再反复踩坑，后续播放该站直接走直连；
- **本站非直连可达（proxy-only）** → 探测 worker 本体：活着 → 只是这条线路过不了，自动换线路保过滤（同样记账）；worker 不通/超时、已有多站代理失败（疑似全局故障/配额耗尽）、或无备选线路 → 回退直连兜底。
